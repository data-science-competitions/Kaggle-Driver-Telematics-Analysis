function [new_trip_structure] = tm_InterpolateTrips(trip_structure,interval,verbose)
%interpolateTrips return trajectories with equally spaced intervals
%   INPUT:
...
for k = 1:K
    ...
    P = interparc(linspace(0,1,ceil(total_length/interval)),X,Y,'linear');
    X = P(:,1);
    Y = P(:,2);
...
if (verbose)
    fprintf(['\n%% Interpolated ',num2str(K),' trips ',num2str(toc),'[sec]\n']);
end
